...
// Computed property untuk cek apakah eligible jadi pemuda
const isEligibleForPemuda = computed(() => {
    if (
        !form.value.tanggal_lahir ||
        !form.value.status ||
        !form.value.status_perkawinan
    ) {
        return false;
    }
    ...
    // Adjust jika belum ulang tahun di tahun ini
    if (
        monthDiff < 0 ||
    ...
    // Cek apakah lansia (>= 60 tahun)
    const isLansia = age >= 60;

    // Check kondisi: umur >= 16 dan < 60 (BUKAN lansia), status = Hidup, status_perkawinan = Belum Menikah
    return (
        age >= 16 &&
        age <= 30 &&
        !isLansia &&
        form.value.status === "Hidup" &&
        form.value.status_perkawinan === "Belum Menikah"
    );
});
...
const openEditModal = (item) => {
    ...
    form.value = {
        ...item,
        // Jika item punya relasi pemuda, ambil jabatan dari sana
        jabatan: item.pemuda?.jabatan || "",
    };
    showModal.value = true;
};
                    ...
                    <div class="overflow-auto max-h-[600px]">
                        <table class="min-w-full divide-y divide-gray-300">
                            ...
                            <tbody class="divide-y divide-gray-200 bg-white">
                                <template v-if="warga.length > 0">
                                    <tr
                                        v-for="(item, index) in warga"
                                        :key="item.id_warga"
                        ...
                        >
                            <div class="text-sm text-gray-700">
                                Menampilkan
                                <span class="font-bold">1</span> hingga
                                <span class="font-bold">10</span> dari
                ...
                <template #default>
                    ...
                    >
                        ...
                        <form @submit.prevent="submitForm" class="space-y-5">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    >
                                        ...
                                        <option
                                            v-for="jk in jenisKelaminOptions"
                                            :key="jk"
                                            :value="jk"
                                        >
                                            {{ jk }}
                            ...
                            <!-- Form Jabatan Pemuda (Conditional) -->
                            <div v-if="isEligibleForPemuda">
                                <label
                                    for="jabatan"
                                    class="block text-sm font-medium text-gray-700 mb-1"
                                >
                                ...
                                >
                                    ...
                                    <option
                                        v-for="jabatan in jabatanPemudaOptions"
                                        :key="jabatan"
                                        :value="jabatan"
                                    >
                                        {{ jabatan }}
                                ...
                                <p class="text-xs text-gray-500 mt-1">
                                    Pilih jabatan jika warga ini aktif di
                                    organisasi pemuda
                                </p>
                            </div>
